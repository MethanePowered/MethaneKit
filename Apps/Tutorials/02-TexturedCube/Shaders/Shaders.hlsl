               ...
               float2 in_uv : TEXCOORD)
{
    ...
    const float3 normal   = normalize(mul(float4(in_normal, 0.0), g_uniforms.model_matrix)).xyz;

    PSInput output;
    output.position       = mul(g_uniforms.mvp_matrix, position);
    output.world_position = mul(g_uniforms.model_matrix, position).xyz;
    output.normal         = normalize(mul(g_uniforms.model_matrix, float4(normal, 0.0)).xyz);
    output.uv             = in_uv;

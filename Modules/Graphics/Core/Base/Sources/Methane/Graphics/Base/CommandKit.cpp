...
CommandKit::CommandKit(const IContext& context, CommandListType cmd_list_type)
    : m_context(context)
...
CommandKit::CommandKit(ICommandQueue& cmd_queue)
...
ICommandQueue& CommandKit::GetQueue() const
{
    META_FUNCTION_TASK();
    if (m_cmd_queue_ptr)
...
bool CommandKit::HasList(CommandListId cmd_list_id) const noexcept
{
    META_FUNCTION_TASK();
...
bool CommandKit::HasListWithState(CommandListState cmd_list_state, CommandListId cmd_list_id) const noexcept
{
    META_FUNCTION_TASK();
    const CommandListIndex cmd_list_index = GetCommandListIndexById(cmd_list_id);
...
ICommandList& CommandKit::GetList(CommandListId cmd_list_id = 0U) const
{
    META_FUNCTION_TASK();
    const CommandListIndex cmd_list_index = GetCommandListIndexById(cmd_list_id);
...
ICommandList& CommandKit::GetListForEncoding(CommandListId cmd_list_id, std::string_view debug_group_name) const
{
    META_FUNCTION_TASK();
...
ICommandListSet& CommandKit::GetListSet(const std::vector<CommandListId>& cmd_list_ids, Opt<Data::Index> frame_index_opt) const
{
    META_FUNCTION_TASK();
...
CommandKit::CommandListIndex CommandKit::GetCommandListIndexById(CommandListId cmd_list_id) const noexcept
{
    META_FUNCTION_TASK();
...
CommandKit::CommandListSetId CommandKit::GetCommandListSetId(const std::vector<CommandListId>& cmd_list_ids, Opt<Data::Index> frame_index_opt) const
{
    META_FUNCTION_TASK();

...
FILE: Methane/Graphics/ProgramArgumentBindingBase.cpp
...
void ProgramArgumentBindingBase::MergeSettings(const ProgramArgumentBindingBase& other)
{
    META_FUNCTION_TASK();
...
ProgramArgumentBindingBase::operator std::string() const
{
    META_FUNCTION_TASK();
    return fmt::format("{} is bound to {}", m_settings.argument, fmt::join(m_resource_views, ", "));
}

bool ProgramArgumentBindingBase::IsAlreadyApplied(const IProgram& program,
